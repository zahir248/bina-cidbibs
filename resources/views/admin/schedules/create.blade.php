                ...
                <div class="card-body">
                    <form action="{{ route('admin.schedules.store') }}" method="POST">
                        ...
                        <div class="form-group mb-2">
                            ...
                            <input type="text" class="form-control @error('session') is-invalid @enderror" id="session" name="session" value="{{ old('session') }}" placeholder="e.g., 1, 2">
                            @error('session')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group mb-4">
                            ...
                            <select class="form-control @error('event_type') is-invalid @enderror" id="event_type" name="event_type" required>
                                ...
                                <option value="facility_management" {{ old('event_type') == 'facility_management' ? 'selected' : '' }}>Facility Management</option>
                                <option value="modular_asia" {{ old('event_type') == 'modular_asia' ? 'selected' : '' }}>Modular Asia</option>
                            </select>
                            @error('event_type')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
